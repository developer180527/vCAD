...
1\. Determinism needs to be cross-platform, and that's a stronger claim than the doc states. The doc talks about determinism as same-input-same-output, which is necessary but not sufficient given your CI matrix already runs five platforms. Determinism within one machine is a much easier bar than determinism across x86/ARM, different compilers, different pdfium builds. Font rasterization and JPEG/JPEG2000 decode in particular have historically had platform-dependent floating-point and SIMD codepaths that produce byte-different (though visually identical) output. I'd make "does pdfium produce byte-identical rasters across your five CI platforms" an explicit sub-question of risk 1, not an assumed corollary — it's entirely possible determinism holds per-machine but not across the matrix, which would force you into per-platform cache namespacing rather than the clean cross-machine sharing story the doc pitches under assetlib. 
github

2\. The "PDF may be genuinely easier" claim on stable identity deserves more skepticism. The doc's optimism rests on a tagged PDF having a logical structure tree to anchor identity to. The problem: a large fraction of real-world PDFs — scanned documents, legacy exports, most things that aren't authored in recent Word/InDesign with accessibility turned on — have no structure tree at all, or one that lies about reading order. For that majority-case document, you're back to anchoring on font+size+position, which is exactly as fragile under reflow as a B-rep face is under a boolean operation. I'd treat "what fraction of real-world PDFs are actually tagged well enough to anchor on" as its own quick empirical spike — pull a representative sample (scanned invoices, old academic PDFs, modern exports) and check — before letting the optimistic framing stand. 
github

3\. Provable redaction has a leak vector orthogonal to the edit graph. The graph model correctly ensures your edits don't retroactively unredact anything. But PDF as a format has its own history mechanism — incremental updates, old object generations, orphaned objects in the xref — that can retain the "removed" content in the source bytes even when your graph says it's gone, if the source file itself already had this baggage before it entered your system. Worth an explicit note (maybe already assumed, but not stated) that provable redaction also requires normalizing/flattening the source's own incremental-update history on ingest, not just correctness of your own edit graph.

4\. Open question 5 is the one I'd actually sit with. You've now got three architecturally serious bets running in parallel — the engine, vCAD, and this. You named the attention question yourself: "is this a second front that starves vCAD." That's not a rhetorical question in the doc, it's a real one, and it's the one piece here that isn't decided by architecture or testable by a spike — it's decided by how much of your actual weekly hours you have. Worth answering it explicitly (a hard percentage-of-time budget, or a trigger condition like "vCAD hits M4 first") before the spike rather than after, since the spike itself will be tempting evidence either way. 
github