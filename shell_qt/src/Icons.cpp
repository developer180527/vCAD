...
// vCAD's own glyph vocabulary. The machinery -- device pixel ratio, pen width, the QIcon itself --
// belongs to proshell::IconSet; only the shapes are here, because "what a fillet looks like" is
// the one part of an icon set that cannot be shared with an application that has no fillets.

const QColor& accentColour() {
    static const QColor c = proshell::IconSet::instance().accent();
    return c;
}
...
/// Paints one of vCAD's glyphs, or returns false for a name that is not ours -- which is how the
/// universal glyphs in proshell keep working without being repeated here.
bool paintCadGlyph(QPainter& g, const QString& name, int size) {
    const QPen pen = g.pen();
    const QColor kLine = pen.color();
    const QColor kAccent = accentColour();
...
void registerCadIcons() {
    proshell::IconSet::instance().addProvider(paintCadGlyph);
}
